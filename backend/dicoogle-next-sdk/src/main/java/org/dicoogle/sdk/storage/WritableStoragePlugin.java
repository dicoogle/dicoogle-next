...
public interface WritableStoragePlugin extends StoragePlugin {
   ...
   *
   * @param data        an {@link InputStream} containing the raw bytes to persist; must not be
   *                    {@code null}
   * @param contentType the MIME type of the data (e.g. {@code "application/dicom"}); may be
   *                    {@code null} if unknown, in which case implementations should default to
   *                    {@code "application/octet-stream"}
   * @return a {@link StoredObject} describing the persisted object, including its URI;
   *         never {@code null}
